...
# 加载训练好的模型
model = Informer()
model.load_state_dict(torch.load(f"../model_stock/price/informer_model_{stock_name}.pth"))  # 使用f-string
model.eval()

# 设置输出文件的位置
output_directory = "../predictions_output/price"
...
def save_predictions_to_csv(dates, predictions, directory, filename):
    df = pd.DataFrame({'Date': dates, 'Price': predictions})
...
plt.plot(dates, predictions, label='Predictions', color='blue')
plt.xlabel('Date')
plt.ylabel('Value')
