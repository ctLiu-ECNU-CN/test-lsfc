...
def create_data(datas):
    ...
    for index in range(0, lens-pre_len-s_len):
        value = datas[index:index+s_len, [0, 1, 3, 4, 5]]
        label = datas[index+s_len-pre_len:index+s_len+pre_len, [0, 2]]
...
def read_data():
    ...
    datas = pd.read_csv(f"../datas/{stock_name}.csv")
    datas.pop("Adj Close")
    datas.fillna(0)

    xs = datas.values[:, [1, 3, 4, 5]]
    ys = datas.values[:, 2]
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            for step, (x, y, xt, yt) in enumerate(test_data):
                ...
                #print("Prediction for batch {}: {}".format(step, final_prediction))
        ...
        model.train()
        high_model_filename = f"../model_stock/high/informer_model_high_{stock_name}.pth"
        torch.save(model.state_dict(), high_model_filename)
