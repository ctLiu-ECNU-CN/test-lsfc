...
# 加载训练好的模型
model = Informer()
model.load_state_dict(torch.load(f"../model_stock/high/informer_model_high_{stock_name}.pth"))  # 使用f-string
model.eval()

# 设置输出文件的位置
output_directory = "../predictions_output/high"
...
def save_predictions_to_csv(dates, predictions, directory, filename):
    df = pd.DataFrame({'Date': dates, 'Predictions_high': predictions})
...
def add_predictions_to_csv(dates, predictions, output_directory, filename):
    ...
    new_data = pd.DataFrame({'Date': dates, 'High': predictions})
...
plt.plot(dates, predictions, label='Predictions_high', color='blue')
plt.xlabel('Date')
plt.ylabel('Value')
