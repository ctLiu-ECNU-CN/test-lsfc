...
# 加载训练好的模型
model = Informer()
model.load_state_dict(torch.load(f"../model_stock/close/informer_model_close_{stock_name}.pth"))  # 使用f-string
model.eval()

# 设置输出文件的位置
output_directory = "../predictions_output/close"
...
plt.plot(dates, predictions, label='Predictions_close', color='grey')
plt.xlabel('Date')
plt.ylabel('Value')
